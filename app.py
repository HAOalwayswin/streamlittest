...
tab1, tab2, tab3 = st.tabs(["광화문·덕수궁", "여의도", "DMC"])
...
with tab1:
    ...
    with tab5:
        ...
        col3, col4 = st.columns([1, 1.5], border=True)
        with col3:
            st.header("상권 주요 정보")
            st.table(main_info)
            st.subheader("성별 비율")
...
with tab2:
    ...
    with tab6:
        if y_response.status_code == 200:
            ...
            if city_data2:
                ...
                col5, col6 = st.columns([1, 1.5], border=True)
                with col5:
    ...
    with tab7:
        ...
        st.title(f"광화문 상권정보 : {y_cmrcl_stts_time.split(' ')[1]}에 갱신")
